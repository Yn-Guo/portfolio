...
const extraEmails = Array.isArray(cfg.extraEmails) ? cfg.extraEmails : [];
for (const [i, em] of extraEmails.entries()) {
  if (em) checkEmail(`extraEmails[${i + 1}]`, em);
}
const extraPhones = Array.isArray(cfg.extraPhones) ? cfg.extraPhones : [];
for (const [i, ph] of extraPhones.entries()) {
  if (!ph) continue;
  if (!/^[+\d\s()./-]{7,20}$/.test(ph)) {
    fail(`extraPhones[${i + 1}]`, `"${ph}" doesn't look like a valid phone number`);
  } else {
    pass(`extraPhones[${i + 1}]`, ph);
  }
}
...
if (skills.length === 0) warn("skills (About cards)", "No categories — About method cards will be empty");
else {
  const total = skills.reduce((n, s) => n + (s.items?.length ?? 0), 0);
  pass("skills (About cards)", `${skills.length} categories · ${total} items`);
}

const skillAreas = cfg.skillAreas || [];
if (skillAreas.length === 0) warn("skillAreas (Skills)", "No areas — Skills section will be empty");
else {
  const total = skillAreas.reduce((n, s) => n + (s.items?.length ?? 0), 0);
  pass("skillAreas (Skills)", `${skillAreas.length} cards · ${total} items`);
}
...
if (blog.enabled) {
  ...
  const blogCategories = Array.isArray(blog.categories) ? blog.categories : [];
  if (blogCategories.length > 0)
    pass("blog.categories", `${blogCategories.length} categories`);
  else
    warn("blog.categories", "No categories — blog filter will fall back to tags");
} else {
