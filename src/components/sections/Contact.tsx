    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col items-center gap-10 text-center">
          ...
          >
            {[config.email, ...config.extraEmails]
              .filter((v): v is string => Boolean(v))
              .map((emailAddress, idx) => (
                <a
                  key={emailAddress}
                  ...
                  {...(idx === 0 ? { 'data-testid': 'link-contact-email' } : {})}
                >
                  <Mail size={15} />
                  {emailAddress}
                </a>
              ))}
            {[config.phone, ...config.extraPhones]
              .filter((v): v is string => Boolean(v))
              .map((phoneNumber, idx) => (
              <a
                key={phoneNumber}
                ...
                {...(idx === 0 ? { 'data-testid': 'link-contact-phone' } : {})}
              >
                <Phone size={15} />
                {phoneNumber}
              </a>
